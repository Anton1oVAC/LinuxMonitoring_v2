...
if [[ $size_sym -eq 1 ]]; then
...
elif [[ $size_sym -eq 4 ]]; then
    count_sym_1=2
    for (( i=1; i<$caunter_generate; i++ )) 
    do
        if [ $count_sym_3 -gt $count_sym_4 ]; then
            let count_sym_4++
            count_sym_3=1
            count_sym_2=1
            count_sym_1=1
    ...
    for (( i=0; i<$( expr $count_sym_1 + $count_sym_2 + $count_sym_3 + $count_sym_4 ); i++ )) 
    do
        if [ $i -lt $count_sym_1 ]; then
            name+="${name_sym:0:1}"
        elif [ $i -lt $( expr $count_sym_1 + $count_sym_2 ) ]; then
    ...
    if [ $size_sym -eq 5 ]; then
        count_sym_6=0
        count_sym_7=0
    elif [ $size_sym -eq 6 ]; then
        count_sym_7=0
    fi
    for (( i=1; i<$caunter_generate; i++ )) 
    do
        if [ $size_sym -eq 7 ] && [ $count_sym_6 -gt $count_sym_7 ]; then
            ...
            count_sym_2=1
            count_sym_1=1
        elif [ $count_sym_4 -gt $count_sym_5 ]; then
            let count_sym_5++
            count_sym_4=1
            count_sym_3=1
            count_sym_2=1
    ...
    do
        if [ $i -lt $count_sym_1 ]; then
            ...
            name+="${name_sym:3:1}"
        elif [ $i -lt $( expr $count_sym_1 + $count_sym_2 \
             + $count_sym_3 + $count_sym_4 + $count_sym_5 ) ]; then
            name+="${name_sym:4:1}"
        elif [ $size_sym -gt 5 ] && [ $i -lt $( expr $count_sym_1 + $count_sym_2 \
             + $count_sym_3 + $count_sym_4 + $count_sym_5 + $count_sym_6 ) ]; then
            name+="${name_sym:5:1}"
